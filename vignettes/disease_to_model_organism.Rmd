---
title: "disease_to_model_organism"
...
Monarch Initiative has indicated that while only 51% of human genes have an annotation associated with them, if one includes annotations of orthologs to those genes in other organisms, 89% have some annotation [Monarch Initiative Nucleic Acids Research paper](doi:10.1093/nar/gkw11128).
...
When we project these mouse interactors to bap1 back to human, we have a projected network of 102 unique human IDs.

Recall we had a network of 98 human IDs using only the human interactions. (97 interactions + bap1 as the hub)
...
Note, we're switching to working with the gene labels rather than IDs.
...
To the extent anything is known of the new genes (the unique to mouse genes) vis a vis cancer or melanoma, these genes might be useful to pursue further. 
Alternatively, if nothing is known with respect to cancer for these genes, they might be interesting as novel targets for discovery.
...
mel_genes <- unique(melanoma$genes$subject)
length(mel_genes)
intersect(mel_genes, uniq_mouse)
```

No new melanoma genes from the mouse network. This isn't too surprising as we relatively 
few genes have been associated with melanoma according to the MONDO ontology.
...
while(check_for_more == TRUE) {
  print(paste("fetching", group, "start", start_row))
...
These 2 genes are Bap1 interactors in mouse. They are not noted as Bap1 interactors in human. 
In human, they are noted as having some association with cancer.
...
We think this sufficiently demonstrates the value of looking at the multiple types of 
annotations as are readily available from Monarch Initiative.
...
TODO: Using all organisms seems to be taking a while. Is this on the server or on my end?
TODO: We haven't seen any server limits in the headers from our previous requests, but it is still on us to play nice.
TODO: However, did we miss any request limits or time limits on subsequent requests?

## response$content$objects might be a handy shortcut for some searches.

We note that in the json  when we searched for cancer genes using the MONDO term, 
cancer$response$content$objects" gets all 3055 unique objects,
